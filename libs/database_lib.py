...
def init_databases(database_name,table_name,drop=False,log=None):
    ...
    if init_database_config is None:
        if log: log.error('No initialisation configuration has been set in <databases> entry')
        return
    if not database_name in init_database_config:
        if log: log.error('No initialisation configuration has been set in <databases> entry for database <%s>'%database_name)
        return
    ...
    if drop:
        if log: log.info('Drop table <%s> on <%s> database'%(table,database_name))
...
def get_entries(database_name,table_name,file_path,configuration,log=None):
    ...
    for database, tables_config in configuration.items():
        ...
        if type(database) == str:
            if database != database_name: continue

            db = core.get_database(database)
            if db is None:
                if log: log.error('In file %s configuration database <%s> is not recognized'%(file_path,database))
                continue
        if db.name != database_name: continue

        if type(tables_config) != dict: 
            if log: log.error('In file %s configuration of database <%s> must be of type <dict>'%(file_path,database))
            continue
        ...
        for table, config in tables_config.items():
            ...
            if not found:
                if log: log.error('In file %s configuration of database <%s> the table <%s> is not found'%(file_path,database,table))
                continue
            
            if 'headers' in config and 'values' in config:
                if type(config['values']) != list: 
                    if log: log.error('In file %s "values" key from table <%s> and database <%s> must be of type <list>'%(file_path,table_name,database))
                    continue
                if type(config['headers']) != list: 
                    if log: log.error('In file %s "headers" key from table <%s> and database <%s> must be of type <list>'%(file_path,table_name,database))
                    continue
                ...
                for entry in config['values']:
                    if type(entry) != list:
                        if log: log.error('In file %s from table <%s> and database <%s> entry <%s> must be of type <list>'%(file_path,table_name,database,entry))
                        continue
                    entries.append(entry)

                if log: log.info('Adding %s entries from <list> for table <%s> in database <%s> from file %s'%(len(entries),table_name,database,file_path))
                process_entries(db,table,log=log,headers=config['headers'],values=entries)
