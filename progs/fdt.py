...
if __name__ == '__main__':
    ...
    if not prod:
        alpha = oracle_lib.Connection('crx033.cro.st.com', '1525', 'fcm','Alphaadm','STCrollesDev_38')
        fcm   = oracle_lib.Connection('crx033.cro.st.com', '1525', 'fcm','fcmadm','STCrollesDev_38')
    else:
        alpha = oracle_lib.Connection('srv-safir', '1525', 'fcm','Alphaadm','Chateauneuf-Du-Pape84')
        fcm   = oracle_lib.Connection('srv-safir', '1525', 'fcm','fcmadm','orafcm11')

    fcm.select("select PHD_ID from PLAGE_HORAIRE_DATES where PHD_DATE_DEBUT > TO_DATE('01/01/2019', 'dd/mm/yyyy')")
