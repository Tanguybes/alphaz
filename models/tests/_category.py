    ...
    def resume(self):
        ...
        for category_name, category in self.categories.items():
            ...
            for group_name, group in category.groups.items():
                ...
                g_content.append(f"{c_space}- {group_name}: {len(g_success)} success, {len(g_failed)} failed, in {g_times}")
            ...
            c_content.append(f"{space}- {category_name}: {len(c_success)} success, {len(c_failed)} failed, in {c_times}")
        ...
        l_content.append(f"- Total: {len(success)} success, {len(failed)} failed, in {times}")
