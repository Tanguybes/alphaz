...
def ask_password_reset(api,username_or_mail,db,close_cnx=True):
    ...
    user_by_username    = user_lib.get_user_data_by_username(db,username_or_mail,close_cnx=False,log=api.log)
...
def try_login(api,db,login, password, ip):
    user_data = user_lib.get_user_data_FromLogin(db,login, password,close_cnx=False,log=api.log)
...
def confirm_user_password_reset(api,token, password, password_confirmation,db):
    ...
    user_data = user_lib.get_user_data_by_password_reset_token(db,token, close_cnx=False,log=api.log)
    if not 'id' in user_data:
        return api.set_error('invalid_token') 
    
    try_reset_password(api,user_data, password, password_confirmation,db=db,log=api.log,close_cnx=True)
...
def try_reset_password(api,user_data, password, password_confirmation,db,log=None,close_cnx=True):
    ...
    valid   = db.execute_query(query,values,close_cnx=False,log=api.log)
    if not valid:
    ...
    values  = (user_data['id'],)
    valid   = db.execute_query(query,values,close_cnx=close_cnx,log=api.log)
    if not valid:
